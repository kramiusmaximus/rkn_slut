...
server_log="$(mktemp -t dnscat2-client-server)"
client_log="$(mktemp -t dnscat2-client-client)"
bundle_log="$(mktemp -t dnscat2-client-bundle)"
stdin_fifo="$(mktemp -u -t dnscat2-client-stdin)"
server_pid=""
...
if ! kill -0 "${server_pid}" 2>/dev/null; then
  cat "${server_log}" >&2
  echo "dnscat2 server exited before the client DNS tunnel test ran" >&2
  exit 1
fi
...
grep -q "Ping response received! This seems like a valid dnscat2 server." "${client_log}" || {
  cat "${server_log}" >&2
  echo "dnscat client failed to establish the DNS tunnel" >&2
  exit 1
}

echo "dnscat client DNS tunnel test passed on ${dns_ip}:${dns_port}."
